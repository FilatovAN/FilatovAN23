function getPrice(quantity, type) 
{
	...
	if (quantity != 0) {
		result.innerHTML = type * quantity + " <зелёных> рублей";
	} else {
...
function getPriceOptions(quantity, type, option) {
	...
	if (quantity == 0) {
		...
		switch (option) {
			case "1":
				result.innerHTML = ((type + 30000) * quantity) + " <зелёных> рублей";
				break;
			case "2":
				result.innerHTML = ((type + 50000) * quantity) + " <зелёных> рублей";
				break;
			case "3":
				result.innerHTML = ((type + 3000) * quantity) + " <зелёных> рублей";
				break;
		}
	}
}
...
window.addEventListener('DOMContentLoaded', function(event) {
	console.log("DOM fully loaded and parsed");
  	let radioListener = "";
	let quantity = 0;
	let typeAll = [10000000, 5000000, 50000000];
	let type = typeAll[0];
	        ...
	        o[0].addEventListener("change", function(event)   
          	{
            	...
            	{
					result.innerHTML = (type + 100000) * quantity + " <зелёных> рублей";
              		f2 = false;
					f3 = false;
		        } 
            	else 
